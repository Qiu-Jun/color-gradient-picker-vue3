...
import { reactive, ref, computed, unref, h, defineComponent, onMounted, onBeforeUnmount, openBlock, createElementBlock, createElementVNode, normalizeStyle, watchEffect, normalizeClass, Fragment, renderList, toDisplayString, createCommentVNode, withDirectives, isRef, vModelText, createVNode, watch, createBlock, withModifiers } from "vue";
...
const getColorObj = (colors) => {
  const idxCols = colors == null ? void 0 : colors.map((c, i) => __spreadProps(__spreadValues({}, c), {
    index: i
  }));
  const upperObj = idxCols == null ? void 0 : idxCols.find((c) => isUpperCase(c.value));
  const ccObj = upperObj || idxCols[0];
  return {
    currentColor: (ccObj == null ? void 0 : ccObj.value) || (config == null ? void 0 : config.defaultGradient),
...
function useColor() {
  ...
  const setValue = (color) => {
    if (!color)
      return;
    const { degreeStr, isGradient: isGradient2 } = getDetails(color);
    setMode(isGradient2 ? Modes.gradient : Modes.solid);
    const colors = getColors(color);
    if (unref(isGradient2)) {
  ...
  const handleGradient = (newColor, left) => {
    ...
    const newColors = [
      { value: newColor.toUpperCase(), left: left != null ? left : currentLeft },
      ...remaining
    ];
  ...
  const setDegrees = (val) => {
    ...
    const value = (_a = colorState.value) == null ? void 0 : _a.split(/,(.+)/)[1];
...
const _hoisted_2$2 = { class: "cpg-controls-item" };
...
const _hoisted_2$1 = { class: "flex justify-start items-center" };
...
const _hoisted_2 = ["data-color"];
const _sfc_main$e = /* @__PURE__ */ defineComponent({
  __name: "Preview",
  setup(__props) {
    const { colorState: colorState2, setValue } = useColor();
    const colorValue = computed(() => (colorState2 == null ? void 0 : colorState2.value) || "");
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        !unref(colorState2).hideOpacity ? (openBlock(), createBlock(unref(_sfc_main$d), {
          key: 5,
          "input-val": unref(colorState2).hc.a * 100,
          label: "A",
          callback: (val) => {
            var _a, _b, _c;
            return unref(handleChange)(`rgba(${(_a = unref(colorState2).hc) == null ? void 0 : _a.r}, ${(_b = unref(colorState2).hc) == null ? void 0 : _b.g}, ${(_c = unref(colorState2).hc) == null ? void 0 : _c.b}, ${val / 100})`);
          }
...
const _sfc_main$6 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const { colorState: colorState2, createGradientStr } = useColor();
    const leftMultiplyer = (colorState2.width - 18) / 100;
    const backgroundImage = computed(() => {
      return colorState2.value ? force90degLinear(colorState2.value) : "";
    });
    const colors = computed(() => colorState2.gradientColors);
    watch(
      () => colors.value,
      ...
      return openBlock(), createElementBlock("div", _hoisted_1$5, [
        ...
        (openBlock(true), createElementBlock(Fragment, null, renderList(unref(colorState2).gradientColors, (point, idx) => {
          return openBlock(), createElementBlock("div", {
            ...
            onMousedown: withModifiers(() => false, ["stop"])
          }, null, 38);
        }), 128))
      ]);
    };
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  __name: "Hue",
  setup(__props) {
    const { colorState: colorState2, handleChange, setHcH } = useColor();
    const canvasRef = ref(null);
    const dragging = ref(false);
    const stopDragging = () => {
      dragging.value = false;
    ...
    const handleHue = (e) => {
      ...
      setHcH(newHue);
      handleChange(`rgba(${r}, ${g}, ${b}, ${colorState2.hc.a})`);
    };
    const handleMove = (e) => {
      if (unref(dragging)) {
        handleHue(e);
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        ...
        !props.hideControls ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [
          createVNode(unref(_sfc_main$g)),
          unref(colorState2).showAdvancedSliders ? (openBlock(), createBlock(unref(_sfc_main$1), { key: 0 })) : createCommentVNode("", true),
          !props.hideGradientControls && unref(isGradient) ? (openBlock(), createBlock(unref(_sfc_main$f), { key: 1 })) : createCommentVNode("", true)
        ], 64)) : createCommentVNode("", true),
        !props.hideGradientControls && unref(isGradient) ? (openBlock(), createBlock(unref(_sfc_main$6), { key: 1 })) : createCommentVNode("", true),
        createVNode(unref(_sfc_main$5)),
        !props.hideOpacity ? (openBlock(), createBlock(unref(_sfc_main$i), { key: 2 })) : createCommentVNode("", true),
        !props.hideInputs ? (openBlock(), createBlock(unref(_sfc_main$7), { key: 3 })) : createCommentVNode("", true),
